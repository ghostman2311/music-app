...
import { DEFAULT_INSTRUMENT, AudioNodesRegistery } from "../../domain/sound";
...
interface Adapted {
  ...
  load(insturment?: InstrumentName): Promise<void>;
...
function useSoundfont({ AudioContext }: Settings): Adapted {
  ...
  async function load(insturment: InstrumentName = DEFAULT_INSTRUMENT) {
    setLoading(true);
    const player = await Soundfont.instrument(audio.current, insturment);
    setLoading(false);
    setCurrent(insturment);
    setPlayer(player);
  }
